...
void main() {
  group('StopwatchScreen Tests', () {
    tearDown(() {
      FakeAsync().flushMicrotasks();
    });

    testWidgets('StopwatchScreen', (widgetTester) async {
  ...
  test('return proper layouts', () async {
    const screen = StopwatchScreen();
    final mobilePortrait = screen.mobilePortrait();
    expect(mobilePortrait, isA<StopwatchMobilePortrait>());
    final mobileLandscape = screen.mobileLandscape();
    expect(mobileLandscape, isA<StopwatchMobileLandscape>());
    final tablet = screen.tablet();
    expect(tablet, isA<StopwatchTablet>());
    final laptopPlus = screen.laptopPlus();
    expect(laptopPlus, isA<StopwatchLaptopPlus>());
  });
}
